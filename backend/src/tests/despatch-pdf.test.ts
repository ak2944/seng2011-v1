...
function requestDespatchAdvicePDF(uuid: string) {
    const response = request('GET', SERVER_URL + `/api/v1/despatch-advice/${uuid}/pdf`, {
        headers: {
...
describe('Retrieve advice', () => {
    test('Valid case: 200', () => {
        const retrieveResponse = requestDespatchAdvicePDF(randomUUID);
        expect(getStatusCode(retrieveResponse)).toStrictEqual(200);
        // Check XML
    });

    test('Invalid xml: 404', () => {
        const retrieveResponse = requestDespatchAdvicePDF('HELLO');
